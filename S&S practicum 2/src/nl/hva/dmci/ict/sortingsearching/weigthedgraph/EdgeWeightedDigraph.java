package nl.hva.dmci.ict.sortingsearching.weigthedgraph;

import java.util.LinkedList;
import nl.hva.dmci.ict.sortingsearching.tileworld.TileType;
import nl.hva.dmci.ict.sortingsearching.tileworld.TileWorldUtil;
...
public class EdgeWeightedDigraph {
 ...
 maakt de adjacency list adj
    ...
    private LinkedList<DirectedEdge> findAdjacent(int v) {
        LinkedList<DirectedEdge> b;
        int x, y;

        TileType tt;
        double diagonalCost, cost;

        x = t.oneDimToTwoDimXCoordinate(v);
        y = t.oneDimToTwoDimYCoordinate(v);

        tt = t.getTileType(x, y);
        diagonalCost = tt.getDiagonalCost();
        cost = tt.getCost();

        b = new LinkedList<DirectedEdge>();
        if (x > 0) {
            // naar Links 
 ...
 in de TileWorldUtil t een coordinaat (x,y) heeft.
    ...
    public void tekenPad(Iterable<DirectedEdge> sp) {
        TileType tt = TileType.PATH;
        int x, y;

        for (DirectedEdge de : sp) {
            x = t.oneDimToTwoDimXCoordinate(de.from());
            y = t.oneDimToTwoDimYCoordinate(de.from());
            t.setTileType(x, y, tt);
        }
    }
